...
**Cost Book built (2026-09-09), per user requirements.** Three sections, all reachable from the dashboard at `/cost-book`: GPM Labor (per-item labor rate for tasks GPM staff perform), Materials (mostly Home Depot — **not yet seeded**, needs the last 6 months of receipts imported; currently starts empty by design rather than fabricated), Vendor Estimates (placeholder rates, explicitly flagged `is_placeholder` until a real vendor quote replaces one for a specific job). Schema: `cost_book_gpm_labor`, `cost_book_materials`, `cost_book_vendor_estimates` (migration `0002_cost_book.sql`).

**Real bugs caught and fixed while building the dashboard (2026-09-09):**
1. `lib/db.ts` opened a live Postgres connection as a module-load side effect. Next.js's build-time "collect page data" phase imports every page module across parallel workers just to inspect exports — each one tried to open a real connection, causing memory exhaustion and worker crashes. Fixed: lazy `getSql()`, called inside each request/action, never at module scope.
2. A dangling `next dev` process (from an earlier session, not actually killed by `kill %1` in Git Bash — a known gap where job control doesn't reliably signal Windows processes) was competing for the same `.next` build directory and caused intermittent, differently-shaped native crashes on `next build` that looked environment-related but weren't. Confirmed by process inspection and killing the correct PIDs via `taskkill`.
3. Date-only Postgres values rendered one day early (`toLocaleDateString()` interpreting UTC midnight in local time). Fixed by forcing UTC in the formatter.
4. Supabase's direct DB connection host (`db.<ref>.supabase.co`) is IPv6-only and unreachable from this network — confirmed via DNS lookup (resolves only to an AAAA record). Required the connection pooler host instead (IPv4-compatible).

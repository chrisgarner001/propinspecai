...
- `/share/[token]` (`app/share/[token]/page.tsx`) must stay publicly reachable, unauthenticated — it's the tenant-facing photo-share link (its own code comment: "Public, unauthenticated view"), gated by its random token, not by login. `app/proxy.ts`'s matcher must explicitly exclude it; it also never got a `requireSession()` call (see Approach A), so it stays open by construction.
  ...
  request │       lib/session.ts decrypt()            │  role check for /setup/*, /cost-book
    ...
    cookie      ▼                         ▼
          redirect /login          NextResponse.next()
                    ...
                    ┌──────────────────────┼──────────────────────┐
                    ▼                      ▼                      ▼
          requireSession/Admin() Server Action           route.ts handler
          (inside each page)
...
4. **`lib/dal.ts`'s `requireSession()`/`requireAdmin()` called directly inside each page component** (revised during implementation, after the Outside Voice pass first proposed a shared `app/(app)/layout.tsx` route-group layout — see below for why that was changed): every one of the 12 protected `page.tsx` files (`app/page.tsx`, `app/inspections/new/page.tsx`, `app/dispatch-board/page.tsx`, `app/inspections/[id]/page.tsx`, `.../chargeback-review/page.tsx`, `.../quote-sheet/page.tsx`, `.../quote-sheet/stages/page.tsx`, `.../stills/page.tsx`, and the 4 Admin-only pages `app/cost-book/page.tsx`, `app/setup/page.tsx`, `app/setup/stages/page.tsx`, `app/setup/users/page.tsx`) calls `await requireSession()` (or `requireAdmin()` for the 4 Admin-only ones) as its first line. This closes the actual read-access hole the Problem Statement opens with, without a shared layout: Next.js's own docs explicitly caution against relying on layout-level checks for exactly this scenario — *"due to Partial Rendering, be cautious when doing checks in Layouts... a layout does not control whether the rest of the route renders... instead, you should do the checks close to your data source or the component that'll be conditionally rendered"* — matching their own separately-documented "Auth checks in page components" pattern instead. This also avoids ~15 risky file moves a `(app)` route group would have required (nested dynamic segments under `inspections/[id]/`), for the identical security outcome. `requireSession()`/`requireAdmin()` redirect on failure (to `/login` or `/`) rather than returning null, since a page has no other way to reject a render.
...
Effort: S-M (human: ~1.5-2 days / CC: ~3-4 hours — `lib/session.ts` + `lib/dal.ts` (`verifySession()`, `requireSession()`, `requireAdmin()`) + `app/proxy.ts`, one `verifySession()` call added to each of ~40 Server Action/Route Handler call sites, `requireSession()`/`requireAdmin()` added to 12 pages, and a role check added to 10 admin-only actions. Revised up from the original 1-day estimate after the Outside Voice pass caught the page-read gap and 6 missed admin actions — both cheap individually, but real work, not a rounding error). Risk: Low.
...
- **Admin-only route list: CONFIRMED** as proposed — `/setup/*` (including `/setup/users`, `/setup/stages`) and `/cost-book`. Dispatch Board stays open to any logged-in user.
- **Session lifetime: CONFIRMED** — 30 days, with silent renewal on activity.
- **Revocation: RESOLVED** by the Approach A correction — deleting or demoting a user in Manage Users takes effect on that user's very next Server Action, Route Handler, or page render (via `requireSession()`/`requireAdmin()` called in each page), through the DAL's live database check.
- **Rate limiting: CONFIRMED deferred** — 3-10 known accounts, not internet-discoverable beyond "knows the URL." Revisit if that changes.
- **Bootstrap path: CONFIRMED** — a one-time seed script (`scripts/seed-admin.mjs`, matching the existing `scripts/migrate.mjs`/`scripts/seed.mjs` convention), reusable if a fresh environment/database is ever needed, rather than a one-off manual `INSERT`.

## Success Criteria
- An unauthenticated request to any page, Server Action, or Route Handler can't do anything (actions/handlers reject without a valid session; pages redirect to `/login` via `requireSession()`/`requireAdmin()` called directly in each page, not just via `proxy.ts`'s optimistic check).
...
[+] requireSession()/requireAdmin() in each of 12 pages (found during Outside Voice pass)
...
Sequential implementation, no parallelization opportunity — `lib/session.ts` → `lib/dal.ts` → `app/proxy.ts` + `requireSession()`/`requireAdmin()` added to 12 pages → the ~40 call-site edits all depend on the same two foundational files existing first, and the call-site edits themselves are mechanical enough (add one import, one line) that splitting them across worktrees would cost more in merge coordination than it saves.
...
- [ ] **T4 (P1, human: ~2h / CC: ~20min)** — routing — Add `requireSession()`/`requireAdmin()` (from `lib/dal.ts`) as the first line of each of the 12 protected page components
  - Surfaced by: Outside Voice finding 1 (page-level read gap); switched from a shared `app/(app)/layout.tsx` to per-page calls after re-reading Next.js's own caution against layout-level auth checks (Partial Rendering)
  - Files: all 12 `page.tsx` files (`app/page.tsx`, `app/inspections/new/page.tsx`, `app/dispatch-board/page.tsx`, `app/inspections/[id]/page.tsx` + 3 sub-pages, `app/cost-book/page.tsx`, `app/setup/page.tsx` + 2 sub-pages) — no file moves

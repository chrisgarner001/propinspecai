...
| 2026-09-10 | Still images are actually extracted from real video via ffmpeg (`scripts/extract-stills.mjs`, midpoint of `source_timestamp`), saved to `dashboard/public/stills/<line_item_id>.jpg`, referenced by `still_image_file` | This is real tenant-property footage, so `public/stills/` stays gitignored (`*.jpg` at repo root already covers it — confirmed with `git status --ignored`) exactly like raw video. It is also NOT uploaded on `vercel deploy` (`.vercelignore` mirrors `.gitignore`), so the hover-preview stills are local-only until a real object-storage backend replaces the public folder — do not remove the gitignore rule to "fix" that |

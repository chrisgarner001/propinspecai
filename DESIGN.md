...
| 2026-09-18 | Fixed: Dispatch Board drag-and-drop wasn't dropping into the calendar in either grouping | Root cause #1: `onDragStart` never called `event.dataTransfer.setData(...)` — several browsers (Firefox reliably, some Chrome/Edge configurations too) silently refuse to treat a drag as a real DnD session without it, so `dragover`/`drop` never fire on any target at all. Root cause #2: day-cells and bars are siblings, not parent/child, and bars had no drop handler of their own — a drop attempted on any spot already covered by an existing bar (common on busy crew rows) landed on the bar instead and was silently rejected, since dragover doesn't bubble sideways to a covered sibling. Fixed by setting real dataTransfer data on every drag source, and by disabling pointer-events on all bars for the duration of a drag (`isDragging` state) so the day-cells underneath stay reachable regardless of what's visually on top of them |

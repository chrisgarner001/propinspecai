...
// batch an item belongs to happens back on the Regular View (its Batch
// column is a plain editable number), not here. This view exists to
// (a) see each batch as a unit before sending it out, and (b) send it to
// PropertyWare.
//
...
export default async function QuoteSheetBatchesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const lineItems = (await sql`
    select id, room_area, item, assigned_to, vendor_id, materials_cost, labor_cost, vendor_estimated_cost, batch_number
    from line_items
  ...
  function batchLabel(items: LineItem[]) {
  ...
  return (
    <AppShell active="/" reviewerName="Jessica Zilka" title="Quote Sheet — Batch View" wide>
      ...
      {batches.size === 0 && (
        <div className="px-6 py-6 text-[13px] text-text-muted">
          No batches yet. Go to Regular View and click &quot;Create Batches&quot;, or set a Batch number on individual items.
        </div>
      )}
            ...
            <div key={batchNumber} className="border-b border-border">
              <div className="flex items-center justify-between gap-4 px-6 py-3 bg-surface-alt">
                <div className="flex items-center gap-3">
                  <span className="font-display font-bold text-[14px]">Batch {batchNumber}</span>
                  <span className="text-[13px] text-text-muted">{batchLabel(items)}</span>
